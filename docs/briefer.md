...
- **Prediction markets page** — Owner's plan is to replace the old Soccer Betting slot with a Polymarket-style page showing what "the market" prices for future events (rate decisions, election outcomes, BTC targets). Slated for the next session.
...
- **Wire the prediction-market page (Polymarket).** Full replacement of the old Soccer Betting slot — a new `/predictions` page pulling odds/probabilities from Polymarket's API through a Vercel serverless function, with the same educational-only compliance framing as `/sectors`. Owner wants this Monday.
- **Strategy layer** — a lightweight "Ntokozo's rating" per JSE ticker (Buy / Watch / Avoid / Sell + one-line reason), stored simply and displayed as a branded card next to the TradingView chart on each stock. Tied to Substack articles. Real editorial differentiation.
...
- Ship a legal footer with T&Cs / privacy / contact.
